...
try {
    if (!isset($_ENV['DO_TOKEN'])) {
        throw new Exception('1st parameter (Access Token) is missing.');
    }

    if (!isset($_ENV['DO_DOMAIN'])) {
        throw new Exception('2nd parameter (Domain) is missing.');
    }

    if (!isset($_ENV['DO_RECORD'])) {
        throw new Exception('3rd parameter (Record) is missing.');
    }

    if (!isset($_ENV['DO_TYPE']) || ($_ENV['DO_TYPE'] != 'AAAA' && $_ENV['DO_TYPE'] != 'A')){
        throw new Exception('4th parameter (Record Type) is missing or incorrect.');
    }

    DEFINE('ACCESS_TOKEN', $_ENV['DO_TOKEN']);         //Digital Ocean Personal Access Tokens (read & write)
    DEFINE('DOMAIN', $_ENV['DO_DOMAIN']);               //joebloggs.co.uk
    DEFINE('RECORD', $_ENV['DO_RECORD']);               //home
    DEFINE('RTYPE', $_ENV['DO_TYPE']);                //A | AAAA
